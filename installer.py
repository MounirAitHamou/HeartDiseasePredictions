...
if globals():
    print("Received missing items:", globals()['missingitems'])
    missingitems = globals()['missingitems']
    currdir = os.getcwd()
    ...
    if 'preprocessor' in missingitems:
    ...
    if 'xtrained' in globals() or 'ytrained' in missingitems:
        print("Creating training data")
        X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.0001, random_state=42)
        from imblearn.combine import SMOTEENN
        from sklearn.model_selection import train_test_split
        smote_enn = SMOTEENN(random_state=42)
        X_train_resampled, y_train_resampled = smote_enn.fit_resample(X_scaled, y)

        np.save('xtrainedresampled.npy',X_train_resampled)
        np.save('ytrainedresampled.npy',y_train_resampled)
        
else:
    print("No missing items received.")
